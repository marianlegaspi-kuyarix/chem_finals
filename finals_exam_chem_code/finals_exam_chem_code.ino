...
#define blueLEDPin 4   // Green LED connected to pin 4 for low CO2
...
void setup() {
  ...
  pinMode(blueLEDPin, OUTPUT);  // Set green LED pin as output
  Serial.begin(9600);
...
void loop() {
  ...
  if (co2ppm > highThreshold) {
    ...
    digitalWrite(blueLEDPin, LOW); // Turn off green LED
  } else if (co2ppm < threshold) {
    digitalWrite(blueLEDPin, HIGH);  // Turn on green LED for low CO2
    ...
    digitalWrite(blueLEDPin, LOW);  // Ensure green LED is off
  }
}
